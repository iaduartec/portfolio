...
export default function UploadPage() {
  ...
  return (
    <Shell className="gap-8">
      <PageIntro
        eyebrow="Datos"
        title="Importar movimientos de cartera"
        description="Flujo recomendado: subir CSV, validar tickers y mercados, y combinar datos en una sesión local lista para portfolio."
        actions={
          <div className="flex flex-wrap items-center gap-3">
            <Button asChild variant="outline">
              <Link href="/portfolio">Ver impacto en portfolio</Link>
            </Button>
            {sessionId ? (
              <span className="rounded-full border border-border/70 bg-surface/65 px-3 py-2 text-xs text-text-secondary">
                Sesión: <span className="financial-value font-semibold text-text">{sessionId}</span>
              </span>
            ) : null}
          </div>
        }
      ...
      <section className="flex flex-col gap-3">
        <SectionHeading
          ...
          description={`Total cargado en local: ${transactions.length}`}
        />
